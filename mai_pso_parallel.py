...
# global vars
n_slave= 8  # the number of particles should be devisible by this number
filename = 'GM51c_SFR2' ## also check file name in the script

slave_folder = os.path.join(os.getcwd(), 'SLAVES')
...
for s in range(n_slave):
    source_folder = os.path.join(os.getcwd(), filename+'_MODFLOW')
    s_folder = os.path.join(slave_folder, f'slave_{s}')

    try:
        shutil.copytree(source_folder, s_folder)
    except FileExistsError:
        print(f'The folder {s_folder} already exists.')
    except Exception as e:
        print(f'An error occurred: {e}')

########### writng run.py script to every slave ############
script_content  ='''# Import modules
import sys
import numpy as np
import pyswarms as ps
import pickle
...
filename = 'GM51c_SFR2'
model  ='MODFLOW-NWT'
environment_file_path = filename+".h5"
...
def mf_simulator2():
    ...
    modflow_nwt_path = model+"_h5_dbl.exe"
...
def read_gage(gage):
    ...
    time_series = pd.read_csv(os.path.join(os.getcwd(), 'model_ts.csv'))
    # print(time_series)
    file_path = os.path.join(os.getcwd(), filename+f'_{gage}.gag')
...
def cost_function(X):
    ...
    for x in X:
        ...
        # read gage_file
        obs_data = pd.read_csv(os.path.join(os.getcwd(), 'observed_gage.csv'))
        # print(obs_data)
        # print('#'*20)
...
X = np.load(os.getcwd()+'/slave_obj.npy')

if __name__ == '__main__':
    obj  =cost_function(X)
    np.save('cost.npy', obj)
'''
...
for s in range(n_slave):
    ...
    if os.path.isdir(s_folder):
        # Define the path to the run.py file in the current folder
        run_file_path = os.path.join(s_folder, 'run.py')

        # Write the script content to the run.py file
        with open(run_file_path, 'w') as file:
            file.write(script_content)

        print(f'Created run.py in slave_{s}')

############################################
###### Cost funtions#################

def distribute_array(original_array, n):
    # Step 1: Flatten the original 2D array
    flattened_array = original_array.flatten()

    # Step 2: Calculate the size of each new array
    total_elements = len(flattened_array)
    base_size = total_elements // n
    extra = total_elements % n
    
    # Step 3: Distribute elements into n smaller arrays
    distributed_arrays = []
    start = 0
    for i in range(n):
        end = start + base_size + (1 if i < extra else 0)
        new_flat_array = flattened_array[start:end]
        # Step 4: Reshape the smaller array
        new_shape = (new_flat_array.size // original_array.shape[1], original_array.shape[1])
        distributed_arrays.append(new_flat_array.reshape(new_shape))
        start = end
    
    return distributed_arrays

# Function to run the `run.py` script in a given folder
def run_script_in_folder(slave_f):
    folder_path = os.path.join(slave_folder, slave_f)  # defined in global var
    script_path = os.path.join(folder_path, 'run.py')
    
    # Navigate to the folder and run the script using subprocess
    try:
        print(f'Running {script_path} in {folder_path}')
        subprocess.run(['python', script_path], cwd=folder_path, check=True)
        print(f'{slave_f}: Script completed successfully.')
    except subprocess.CalledProcessError as e:
        print(f'{slave_f}: Script failed with error: {e}')

# def run_script_in_folder(slave_f):
#     folder_path = os.path.join(slave_folder, slave_f)  # Assume slave_folder is defined globally
#     script_path = os.path.join(folder_path, 'run.py')
...
#         process = subprocess.Popen(command, cwd=folder_path, shell=True)
#         process.wait()

#         print(f'{slave_f}: Script started in a new Command Prompt window.')
#     except Exception as e:
#         print(f'{slave_f}: Script failed with error: {e}')
...
def cost_function(X):
    ...
    """
    ## distribute the X to each slave
    slaves_X= distribute_array(X, n_slave)

    for s in range(n_slave):
        s_folder = os.path.join(slave_folder, f'slave_{s}')
        np.save(os.path.join(s_folder, 'slave_obj.npy'), slaves_X[s])

    #### running the Run.py in each slave folder ####

    # List all folders in the working directory
    folders = [f for f in os.listdir(slave_folder) if os.path.isdir(os.path.join(slave_folder, f))]

    # Create a thread for each folder to run the script
    threads = []
    for folder in folders:
        thread = threading.Thread(target=run_script_in_folder, args=(folder,))
    ...
    print("All slaves have been executed.")

    ##### reading the rmse in each slave and appending in one array #########
    cost = np.array([])
    for s in range(n_slave):
        arr_path = os.path.join(slave_folder, f'slave_{s}' ,'cost.npy')
        s_cost = np.load(arr_path)
        cost = np.append(cost, s_cost)
...
cost, pos = optimizer.optimize(
    ...
    iters=500 # Maximum number of iterations
